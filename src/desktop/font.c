 ...
 *
 * All drawing goes through fb_put_pixel() so it participates in the
...
static const uint8_t font_glyphs[GLYPH_COUNT][FONT_HEIGHT] = {
...
const uint8_t *font_get_glyph(char c)
{
    if (c < GLYPH_FIRST || c > GLYPH_LAST)
        return font_glyphs[0]; /* space for unprintable */
    return font_glyphs[(uint8_t)c - GLYPH_FIRST];
}
                          ...
                          uint32_t fg, uint32_t bg)
{
    ...
    while (*str) {
        ...
        case '\n':
            ...
            break;
        case '\t':
            /* Advance to next 4-char tab stop */
            x = start_x + (((x - start_x) / FONT_WIDTH + 4) & ~3u) * FONT_WIDTH;
            break;
        case '\r':
            x = start_x;
            break;
...
uint32_t font_text_width(const char *str)
{
    ...
    while (*str) {
        if (*str == '\t')
            w = ((w / FONT_WIDTH + 4) & ~3u) * FONT_WIDTH;
        else if (*str != '\n' && *str != '\r')
            w += FONT_WIDTH;
        str++;
    }
...
uint32_t font_char_height(void) { return FONT_HEIGHT; }
uint32_t font_char_width(void)  { return FONT_WIDTH; }
